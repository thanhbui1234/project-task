...
export const useCreatTask = (projectId: string) => {
  const { mutate: createTask, isPending } = useMutation<void, Error, ICreateTaskSchema, void>({
    mutationFn: (data: ICreateTaskSchema) => api.post(API_ENDPOINTS.CREATE_TASK, { projectId, ...data }),
    onSuccess: () => {
